...
# /data/jgordon/old-bailey/old-bailey-split.tsv
...
word2id = dict(
    [(w, cvectorizer.vocabulary_.get(w)) for w in cvectorizer.vocabulary_]
)
id2word = dict(
    [(cvectorizer.vocabulary_.get(w), w) for w in cvectorizer.vocabulary_]
)
del cvectorizer
...
timestamps_tr = [
    time2id[timestamps[idx_permute[idx_d]]] for idx_d in range(trSize)
]
docs_ts = [
    [
        word2id[w]
        for w in docs[idx_permute[idx_d + trSize]].split()
        if w in word2id
    ]
    for idx_d in range(tsSize)
]
timestamps_ts = [
    time2id[timestamps[idx_permute[idx_d + trSize]]] for idx_d in range(tsSize)
]
docs_va = [
    [
        word2id[w]
...
timestamps_va = [
    time2id[timestamps[idx_permute[idx_d + trSize + tsSize]]]
    for idx_d in range(vaSize)
]
del docs

print(
    "  number of documents (train): {} [this should be equal to {} and {}]".format(
        len(docs_tr), trSize, len(timestamps_tr)
    )
)
print(
    "  number of documents (test): {} [this should be equal to {} and {}]".format(
        len(docs_ts), tsSize, len(timestamps_ts)
    )
)
print(
    "  number of documents (valid): {} [this should be equal to {} and {}]".format(
...
docs_ts_h1 = [
    [w for i, w in enumerate(doc) if i <= len(doc) / 2.0 - 1]
    for doc in docs_ts
]
docs_ts_h2 = [
...
bow_ts_h1 = create_bow(
    doc_indices_ts_h1, words_ts_h1, n_docs_ts_h1, len(vocab)
)
bow_ts_h2 = create_bow(
    doc_indices_ts_h2, words_ts_h2, n_docs_ts_h2, len(vocab)
)
...
write_lda_file(
    path_save + "dtm_ts_h1-mult.dat", timestamps_ts, time_list, bow_ts_h1
)
write_lda_file(
    path_save + "dtm_ts_h2-mult.dat", timestamps_ts, time_list, bow_ts_h2
)
...
savemat(
    path_save + "bow_tr_timestamps.mat",
    {"timestamps": timestamps_tr},
    do_compression=True,
)
savemat(
    path_save + "bow_ts_timestamps.mat",
    {"timestamps": timestamps_ts},
    do_compression=True,
)
savemat(
    path_save + "bow_va_timestamps.mat",
    {"timestamps": timestamps_va},
    do_compression=True,
)
...
savemat(
    path_save + "bow_tr_tokens.mat",
    {"tokens": bow_tr_tokens},
    do_compression=True
)
savemat(
    path_save + "bow_tr_counts.mat",
...
savemat(
    path_save + "bow_ts_tokens.mat",
    {"tokens": bow_ts_tokens},
    do_compression=True
)
savemat(
    path_save + "bow_ts_counts.mat",
...
savemat(
    path_save + "bow_ts_h1_tokens.mat",
    {"tokens": bow_ts_h1_tokens},
    do_compression=True,
)
savemat(
    path_save + "bow_ts_h1_counts.mat",
    {"counts": bow_ts_h1_counts},
    do_compression=True,
)
del bow_ts_h1
...
savemat(
    path_save + "bow_ts_h2_tokens.mat",
    {"tokens": bow_ts_h2_tokens},
    do_compression=True,
)
savemat(
    path_save + "bow_ts_h2_counts.mat",
    {"counts": bow_ts_h2_counts},
    do_compression=True,
)
del bow_ts_h2

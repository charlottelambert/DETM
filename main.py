...
from utils import get_topic_coherence, nearest_neighbors
...
parser.add_argument(
    "--data_path", type=str, default="un/", help="directory containing data"
)
parser.add_argument(
    "--emb_path",
    type=str,
    default="skipgram/embeddings.txt",
...
parser.add_argument(
    ...
    type=int,
    default=1000,
    help="number of documents in a batch for training",
)
parser.add_argument(
    "--min_df",
    type=int,
    default=100,
    help="to get the right data..minimum document frequency",
)

# Model-related arguments
parser.add_argument(
    "--num_topics", type=int, default=50, help="number of topics"
)
parser.add_argument(
    "--rho_size", type=int, default=300, help="dimension of rho"
)
parser.add_argument(
...
parser.add_argument(
    "--delta", type=float, default=0.005, help="prior variance"
)
...
parser.add_argument(
    "--mode", type=str, default="train", help="train or eval model"
)
parser.add_argument(
    "--optimizer", type=str, default="adam", help="choice of optimizer"
)
parser.add_argument(
    "--seed", type=int, default=2020, help="random seed (default: 1)"
)
parser.add_argument(
...
parser.add_argument(
    ...
    type=float,
    default=0.0,
    help="dropout rate on rnn for eta",
)
parser.add_argument(
    "--clip", type=float, default=0.0, help="gradient clipping"
)
parser.add_argument(
...
vocab, train, valid, test = data.get_data(args.data_path, temporal=True)
...
print("=*" * 40)
print(
    "Training a DETM on {} with the following settings: {}".format(
        ...
        for k in range(args.num_topics):
            for t in times:
                try:
                    ...
                    print("- Topic {} at time {}:\n  {}".format(k, t, topic_words))
                except IndexError:
                    continue
        print("\n")
        print("Visualize word embeddings ...")
        queries = ["man", "woman", "murder", "theft", "rape", "hanged", "london", "highway"]
        try:
        ...
        for word in queries:
            try:
                print(
                    "- '{}' neighbors:\n  {}".format(
                        word,
                        nearest_neighbors(word, embeddings, vocab, args.num_words),
                    )
                )
            except ValueError:
                continue
...
def _eta_helper(rnn_inp):
    ...
    inp_0 = torch.cat(
        [output[0], torch.zeros(model.num_topics).to(device)], dim=0
    )
...
def get_completion_ppl(source):
    ...
    with torch.no_grad():
        ...
        if source == "val":
            ...
            print("*" * 80)
            print("{} Document completion perplexity: {}".format(source.upper(), ppl_dc))
            print("*" * 80)
            return ppl_dc

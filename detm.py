...
class DETM(nn.Module):
    def __init__(self, args, embeddings):
        ...
        # Define the variational parameters for the topic embeddings over time
        # (alpha) ... alpha is K x T x L
        self.mu_q_alpha = nn.Parameter(
            torch.randn(args.num_topics, args.num_times, args.rho_size)
        )
        self.logsigma_q_alpha = nn.Parameter(
            torch.randn(args.num_topics, args.num_times, args.rho_size)
        )

        # Define variational distribution for \theta_{1:D} via
        # amortizartion... theta is K x D
        ...
        self.mu_q_theta = nn.Linear(
            args.t_hidden_size, args.num_topics, bias=True
        )
        self.logsigma_q_theta = nn.Linear(
            args.t_hidden_size, args.num_topics, bias=True
        )

        # Define variational distribution for \eta via amortizartion... eta
        # is K x T
        self.q_eta_map = nn.Linear(args.vocab_size, args.eta_hidden_size)
        self.q_eta = nn.LSTM(
            args.eta_hidden_size,
            args.eta_hidden_size,
            args.eta_nlayers,
            dropout=args.eta_dropout,
        )
        self.mu_q_eta = nn.Linear(
            args.eta_hidden_size + args.num_topics, args.num_topics, bias=True
        )
    ...
    def get_alpha(self):  # mean field
        ...
        for t in range(1, self.num_times):
            ...
            logsigma_p_t = torch.log(
                self.delta
                * torch.ones(self.num_topics, self.rho_size).to(device)
            )
            kl_t = self.get_kl(
                self.mu_q_alpha[:, t, :],
    ...
    def get_eta(self, rnn_inp):  # structured amortized inference
        ...
        inp_0 = torch.cat(
            [output[0], torch.zeros(self.num_topics).to(device)], dim=0
        )
        ...
        for t in range(1, self.num_times):
            ...
            p_mu_t = etas[t - 1]
            logsigma_p_t = torch.log(
                self.delta * torch.ones(self.num_topics).to(device)
            )
    ...
    def get_theta(self, eta, bows, times):  # amortized inference
        ...
        kl_theta = self.get_kl(
            ...
            logsigma_theta,
            eta_td,
            torch.zeros(self.num_topics).to(device),
        )
    ...
    def init_hidden(self):
        """Initializes the first hidden state of the RNN used as inference
        network for eta."""
